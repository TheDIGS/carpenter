...
module.exports = class extends (
	...
	async run(message) {
		...
		if (Date.now() - userStats.levels.lastMessage > 0) {
			let newXP =
				userStats.levels.experience + require('random').int(15, 25);
			...
			const xpToNextLevel =
				5 * Math.pow(userStats.levels.level, 2) +
				50 * userStats.get('levels.level') +
				100;
			...
			if (userStats.levels.experience >= xpToNextLevel) {
				...
				message.send(
					`**${message.author}** has reached level **${userStats.levels.level}**!`
				);
			}
		}
	}
